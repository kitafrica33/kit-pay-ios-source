/*
 * Copyright 2026 LiveKit
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#if os(iOS)

import Foundation

#if canImport(UIKit)
import UIKit
#endif

internal import LiveKitWebRTC

class BroadcastScreenCapturer: BufferCapturer, @unchecked Sendable {
    private let appAudio: Bool
    private let socketPath: SocketPath?
    private let lifecycle = SerialRunnerActor<Bool>()

    private struct ReceiverState {
        var generation: UUID?
        var task: AnyTaskCancellable?
        var join: (@Sendable () async -> Void)?
        var receiver: BroadcastReceiver?
    }

    private let receiverState = StateSync(ReceiverState())

    deinit {
        let pending = receiverState.copy()
        pending.task?.cancel()
        pending.receiver?.close()
    }

    override func startCapture() async throws -> Bool {
        try await lifecycle.run { [self] in
            try await startCaptureSerially()
        }
    }

    private func startCaptureSerially() async throws -> Bool {
        let didStart = try await super.startCapture()

        guard didStart else { return false }

        let bounds = await UIScreen.main.bounds
        let width = bounds.size.width
        let height = bounds.size.height
        let screenDimension = Dimensions(width: Int32(width), height: Int32(height))

        // pre fill dimensions, so that we don't have to wait for the broadcast to start to get actual dimensions.
        // should be able to safely predict using actual screen dimensions.
        let targetDimensions = screenDimension
            .aspectFit(size: options.dimensions.max)
            .toEncodeSafeDimensions()

        set(dimensions: targetDimensions)
        guard createReceiver() else {
            _ = try await super.stopCapture()
            return false
        }
        return true
    }

    private func createReceiver() -> Bool {
        guard let socketPath else {
            log("Bundle settings improperly configured for screen capture", .error)
            return false
        }
        let generation = UUID()
        receiverState.mutate { $0.generation = generation }
        let task = Task { [weak self, appAudio] in
            do {
                try Task.checkCancellation()
                let receiver = try await BroadcastReceiver(socketPath: socketPath)
                defer { receiver.close() }
                try Task.checkCancellation()
                guard self?.receiverState.mutate({ state in
                    guard state.generation == generation else { return false }
                    state.receiver = receiver
                    return true
                }) == true else { return }
                self?.log("Broadcast receiver connected", .debug)

                if appAudio {
                    try await receiver.enableAudio()
                }

                for try await sample in receiver.incomingSamples {
                    try Task.checkCancellation()
                    guard let self, receiverState.generation == generation else { break }
                    capture(sample)
                }
                self?.log("Broadcast receiver closed", .debug)
            } catch {
                if !Task.isCancelled {
                    self?.log("Broadcast receiver error: \(error)", .error)
                }
            }
            if !Task.isCancelled {
                // Schedule retirement after this task returns; retirement joins this task.
                Task.discarding { [weak self] in
                    try await self?.receiverDidFinish(generation: generation)
                }
            }
        }
        receiverState.mutate {
            $0.task = task.cancellable()
            $0.join = { await task.value }
        }
        return true
    }

    private func capture(_ sample: BroadcastReceiver.IncomingSample) {
        switch sample {
        case let .image(buffer, rotation): capture(buffer, rotation: rotation)
        case let .audio(buffer): AudioManager.shared.mixer.capture(appAudio: buffer)
        }
    }

    override func stopCapture() async throws -> Bool {
        try await lifecycle.run { [self] in
            try await stopCaptureSerially()
        }
    }

    private func stopCaptureSerially() async throws -> Bool {
        let didStop = try await super.stopCapture()

        // Already stopped
        guard didStop else { return false }
        let pending = receiverState.mutate { state in
            let pending = state
            state = ReceiverState()
            return pending
        }
        pending.task?.cancel()
        pending.receiver?.close()
        await pending.join?()
        return true
    }

    private func receiverDidFinish(generation: UUID) async throws {
        _ = try await lifecycle.run { [self] in
            guard receiverState.generation == generation else { return false }
            return try await stopCaptureSerially()
        }
    }

    init(delegate: LKRTCVideoCapturerDelegate,
         options: ScreenShareCaptureOptions,
         socketPath: SocketPath? = BroadcastBundleInfo.socketPath)
    {
        appAudio = options.appAudio
        self.socketPath = socketPath
        super.init(delegate: delegate, options: BufferCaptureOptions(from: options))
    }
}

public extension LocalVideoTrack {
    /// Creates a track that captures screen capture from a broadcast upload extension
    static func createBroadcastScreenCapturerTrack(name: String = Track.screenShareVideoName,
                                                   source: Track.Source = .screenShareVideo,
                                                   options: ScreenShareCaptureOptions = ScreenShareCaptureOptions(),
                                                   reportStatistics: Bool = false) -> LocalVideoTrack
    {
        let videoSource = RTC.createVideoSource(forScreenShare: true)
        let capturer = BroadcastScreenCapturer(delegate: videoSource, options: options)
        return LocalVideoTrack(
            name: name,
            source: source,
            capturer: capturer,
            videoSource: videoSource,
            reportStatistics: reportStatistics,
        )
    }
}

#endif
