...
| Messaging | Conversation UI, account-bound encrypted drafts/history, payment-request timeline cards, recipient/tag search, Signal v2 PQXDH/Double Ratchet delivery, fail-closed offline outbox, versioned privacy choices and directional blocking; compatible iPhone users also have 200 MiB voice/video/document media (offline-first for every kind), pinned/muted filters, multi-select, forwarding, per-chat search, in-app document viewing, encrypted iCloud backup/restore/delete, and chat-native payment events (sent/received transfers with capability-gated Accept/Reject/Reverse, payment requests and their responses); encrypted capability-gated reactions, server-authoritative group create/rename/add/remove/leave with lifecycle notices and read-only fallback, and Reverb-backed sync nudges plus account-private presence/typing are implemented behind their server gates, with 200 MiB media, groups, reactions, and Reverb fenced to iOS 1.0.16 build 24 (`1.0.16-r24`) or later | Add bounded durable recovery for the narrow post-transfer/pre-chat-queue crash interval; provision and validate production CloudKit, privacy disclosures, and two-device backup/restore/delete before physical acceptance. Group reporting remains blocked because the two-party abuse-report API cannot safely stand in for it. |
