...
| Scheduled and group payments | Server-authoritative direct/group schedules plus collaborative group requests with partial or full contributions, exact 100% closure, authoritative contributor attribution, stable idempotency keys, and intent-bound step-up are implemented behind exact protocol capability blocks | Fence all 11 new sync event types to iOS `1.0.16-r39`+, keep every event-referenced resource readable to entitled recipients, deploy the matching endpoints/capabilities, and validate execution/cancellation/relaunch on physical devices before enabling the gates |
...
| Messaging | Conversation UI, account-bound encrypted drafts/history, payment-request timeline cards, recipient/tag search, Signal v2 PQXDH/Double Ratchet delivery, fail-closed offline outbox, device-local Send Later with durable background/foreground recovery, versioned privacy choices and directional blocking; compatible iPhone users also have 200 MiB voice/video/document media (offline-first for every kind), pinned/muted filters, multi-select, forwarding, per-chat search, in-app document viewing, encrypted iCloud backup/restore/delete with user-confirmed damaged-backup replacement and automatic daily/weekly/monthly catch-up, and chat-native payment events (sent/received transfers with capability-gated Accept/Reject/Reverse, payment requests and their responses); immediate transfer, payment-request create/pay/cancel, group-payment, and group-contribution cards cross an account-bound protected pre-POST journal into the deterministic E2EE outbox, with bounded exact-endpoint recovery that never scans history or replays a mutation; encrypted capability-gated reactions, server-authoritative group create/rename/add/remove/leave with lifecycle notices and read-only fallback, in-app reporting of authenticated direct or group-message senders, and Reverb-backed sync nudges plus account-private presence/typing are implemented behind their server gates, with 200 MiB media, groups, reactions, and Reverb fenced to iOS 1.0.16 build 24 (`1.0.16-r24`) or later | Validate interruption/relaunch recovery for every financial chat event, best-effort Send Later, and automatic-backup timing after process termination/expiration/offline recovery; provision and validate production CloudKit, privacy disclosures, and two-device backup/restore/delete before physical acceptance. |
...
- Add configurable read receipts, disappearing messages, group reporting,
