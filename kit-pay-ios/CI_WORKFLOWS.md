...
Build 104 fixes a second, unrelated crash. Build 102 opened chats correctly but was killed
...
Build 103's archive died on the macOS test stage because that native case also probed for the
*released* lock from the assertion's end handler, which the test host does not report reliably;
the ordering is pinned by the source contract instead. The corrected source needed a new tag —
a published release's tag is burned — so the build moved to 104.
