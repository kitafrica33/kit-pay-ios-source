...
final class KitCameraController: NSObject, ObservableObject, @unchecked Sendable {
    ...
    /// Bound ordinary chat recordings at capture time instead of recording a large 4K source
    /// and making the sender wait for a second export. Older/front cameras keep a valid fallback.
    private func applyPreferredCapturePreset() {
    ...
    func flipCamera() {
        sessionQueue.async { [weak self] in
            ...
            // The replacement camera may not support the current camera's resolution.
            ...
            self.applyPreferredCapturePreset()
