...
enum MessageBackupError: LocalizedError, Equatable {
    ...
    var errorDescription: String? {
        ...
        case .invalidBackup:
            "This backup could not be decrypted on this iPhone."
        case .backupNotFound:
...
enum MessageBackupCrypto {
    ...
    static func decrypt(_ data: Data, key: Data) throws -> MessageBackupPayload {
        ...
        do {
            ...
            let clear = try ChaChaPoly.open(sealed, using: SymmetricKey(data: key))
            let decoder = JSONDecoder()
            decoder.dateDecodingStrategy = .iso8601
            let payload = try decoder.decode(MessageBackupPayload.self, from: clear)
            try MessageBackupValidationPolicy.validate(payload)
            return payload
        } catch let error as MessageBackupError {
            throw error
        } catch {
            throw MessageBackupError.invalidBackup
        }
